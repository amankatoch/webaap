...
angular.module('myBathroomApp')
  .controller('TilingCtrl', function ($scope,$http,$q,$cookies,$cookieStore,$location,$timeout,products,ngDialog) {
    $scope.awesomeThings = [
      'HTML5 Boilerplate',
      'AngularJS',
      'Karma'
...
$.each(products_cat,function(data1,val1){
  ...
  $scope.$apply();
    ...
    var setres=products.settile($scope.prod);
    $cookieStore.put('Tile', $scope.prod);
    $(".ngdialog-overlay").click();

   $location.path("/toilet");
  }
